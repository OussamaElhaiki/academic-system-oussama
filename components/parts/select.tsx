...
export function Select(props: IProps) {
  ...
  return (
    <>
      <label htmlFor={selProps.name} className="block mb-2 text-sm font-medium text-gray-900">
        {selProps.label}
      </label>
      <select
      ...
      {selProps.error && (
        <div className="mt-1 col-span-2 p-1 bg-red-100 italic text-sm">{selProps.error}</div>
      )}
    </>
  )
}