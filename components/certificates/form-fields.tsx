...
const initialState: IState = { message: "", errors: undefined, isSaved: false }
...
export function FormFields(props: IProps) {
  ...
  const [state, formAction] = useActionState<IState, FormData>(createCertificates, initialState)
  ...
  const handleAction = (data: FormData) => {
    formAction(data)
    ref.current?.reset()
    if (data.has("id")) setEditCert(undefined)
  }

  return (
    <form ref={ref} action={handleAction} className="grid gap-y-5 max-w-md">
      <div className="grid grid-cols-2">
        <Select options={toSelArr<ICertType>(certTypes, "title")} selProps={selProps} />
      </div>
      <div className="grid grid-cols-2">
        <TextField
          label="Note"
      ...
      <div className={`my-2 text-sm italic p-1 ${state?.errors ? "bg-red-100" : state?.message ? "bg-green-100" : ""}`}>
        {state?.message}
      </div>
